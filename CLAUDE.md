...
## Status (as of 23 May 2026)

- ✅ **PicoMuon detector**: delivered, on desk, ready to use
- 📦 **Enviro Weather board**: ordered, awaiting delivery
- 📦 **2× AA battery holder + JST-PH cable**: ordered
- ⏳ **TFA Stevenson screen**: not yet ordered
- ⏳ **AA NiMH rechargeables + charger**: not yet ordered
- ✅ **Pi 4 + heatsink + PSU + 32GB microSD + USB sticks**: already on hand
- ⚠️ **Pi 4 currently has leftover setup from a previous project** — must be wiped and reinstalled from scratch as part of Weekend 1 (fresh Pi OS Lite image, hostname `observatory`, no carry-over config or services).

Build can start immediately on Pi 4 setup (Weekend 1). Weather node provisioning (Weekend 2) waits for Enviro Weather + Stevenson screen delivery.
