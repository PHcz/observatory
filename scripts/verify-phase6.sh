...
if [ "$SKIP_MANUAL" -eq 1 ]; then
  ...
  echo ""
  echo "  This criterion verifies that a browser-based WS client reconnects"
  echo "  automatically after a network interruption."
  echo ""
  echo -e "${YELLOW}  RECOMMENDED: SKIP at Phase 6 acceptance.${NC}"
  echo ""
  echo "  Why: Phase 6 ships the WS endpoint, not a client. The current"
  echo "  static placeholder at / has no JavaScript, and the Python REPL"
  echo "  (\`python -m websockets ...\`) has no auto-reconnect logic — so"
  echo "  neither can actually demonstrate browser reconnect behavior."
  echo "  The reconnect contract is implemented + verified during Phase 7"
  echo "  (SvelteKit dashboard with exponential-backoff WS client)."
  echo ""
  echo "  If you DO have a Phase 7 dashboard deployed and want to test now:"
  echo "    1. Open http://observatory.local:8000/ on a Mac/laptop on the"
  echo "       same wifi."
  echo "    2. Open DevTools > Console (F12)."
  echo "    3. Toggle the laptop's wifi OFF for 5 seconds, then ON."
  echo "    4. Within 5–10s of wifi resuming, the console should log a"
  echo "       new 'WebSocket connected' / 'snapshot received' entry."
  echo "    5. Reply 'pass' if reconnect succeeded, 'fail' otherwise."
  echo ""
  echo "  Otherwise: reply 'skip' (this is the default — just press Enter)."
  echo ""
  echo -e "${YELLOW}===== CRITERION 3 (operator action) =====${NC}"
  echo ""
  read -r -p "Result [skip (default) / pass / fail]: " OP_REPLY || OP_REPLY="skip"
  OP_REPLY="${OP_REPLY,,}"  # lowercase
  [ -z "$OP_REPLY" ] && OP_REPLY="skip"  # empty input → skip
