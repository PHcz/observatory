...
# --- Phase 13 NMDB neutron-monitor poller (hourly oneshot, no Restart=) ---


def test_nmdb_service_unit() -> None:
    unit = DEPLOY / "obs-nmdb-poll.service"
    assert unit.exists(), f"missing {unit}"
    text = unit.read_text()
    assert "Type=oneshot" in text
    assert "User=observatory" in text
    assert "Group=observatory" in text
    assert "EnvironmentFile=/etc/observatory/observatory.env" in text
    assert "ExecStart=/opt/observatory/.venv/bin/python -m observatory.pollers.nmdb" in text
    assert "SyslogIdentifier=obs-nmdb-poll" in text
    assert "After=network-online.target chrony.service time-sync.target" in text
    # oneshot must NOT carry Restart= — the hourly timer IS the retry (RESEARCH Anti-Pattern)
    assert "Restart=" not in text, "oneshot unit must NOT have Restart= directive"


def test_nmdb_timer_unit() -> None:
    unit = DEPLOY / "obs-nmdb-poll.timer"
    assert unit.exists(), f"missing {unit}"
    text = unit.read_text()
    assert "Requires=obs-nmdb-poll.service" in text
    assert "OnUnitActiveSec=1h" in text, "NMDB timer fires hourly"
    assert "RandomizedDelaySec=30" in text, "POLL-08 requires RandomizedDelaySec=30"
    assert "Persistent=true" in text, "missed firings after reboot must catch up"
    assert "WantedBy=timers.target" in text


def test_bootstrap_enables_nmdb_timer_with_migration_reminder() -> None:
    script = (Path(__file__).resolve().parents[2] / "scripts" / "bootstrap-pi.sh").read_text()
    assert "systemctl enable obs-nmdb-poll.timer" in script
    assert "obs-nmdb-poll.service" in script
    # 0007 migration reminder (obs-api does not auto-migrate — Phase 10 deploy lesson)
    assert "apply_migrations" in script
